...
module.exports = function({ generators, extensions }) {
	return function(
		pages = [],
		...
		Object.keys(templates).forEach((resourcePath) => {
			...
			const importeeOutPath =
				constants.COMPONENT_OUTPUT_PATH
					.replace(/\[name\]/g, compilerOptions.name)
					.replace(/\[root\]/g, importeeSubpackage ? importeeSubpackage.root + '/' : '') + extensions.template
			...
			imports['_slots_'] = {
				name: '',
				src: relativeToRoot(importeeOutPath) + constants.SLOTS_OUTPUT_PATH + extensions.template
			}
			...
			if (compiledComponentTemplate.md5 !== md5) {
				let compiler = megaloTemplateCompiler[useCompiler] || megaloTemplateCompiler.vue

				const { body, slots, needHtmlParse } = generators.component({
					source,
					compiler,
					compilerOptions: Object.assign({}, compilerOptions, { target: platform, imports, htmlParse })
				})
				...
				if (htmlParse && needHtmlParse) {
					// add htmlparse
					const htmlPraserSrc =
						relativeToRoot(importeeOutPath) + constants.HTMLPARSE_TEMPLATE_OUTPUT_PATH + extensions.template
				...
				// emit component
				emitFile(importeeOutPath, finalBody, compilation)
			} else {
...
function normalizeGeneratorOptions(options, { constants, extensions }) {
	...
	entryComponent.src =
		constants.COMPONENT_OUTPUT_PATH
			...
			.replace(/\[name\]/g, entryComponent.name) + extensions.template

	return options
}

function normalizeImports({ imports = {}, importeeOutPath, extensions = {}, subpackages = [] }) {
	...
	Object.keys(imports).forEach((k) => {
		...
		const src =
			relativeToRoot(importeeOutPath) +
			constants.COMPONENT_OUTPUT_PATH.replace(/\[root\]/g, root).replace(/\[name\]/g, name) +
			extensions.template
